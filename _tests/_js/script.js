describe('integration test', () => {
    beforeAll(async () => {
        await page.goto('https://example-auth-page1.local.vielhuber.de', { waitUntil: 'networkidle2' });
    }, 3000);
    ...
    test('overall behaviour', async () => {
        ...
        await expect(page).toMatch('null');
        await page.goto('https://example-auth-page3.local.vielhuber.de', { waitUntil: 'networkidle2' });
        await page.waitForSelector('.status.null');
        await expect(page).toMatch('null');
    }, 10000);
});
